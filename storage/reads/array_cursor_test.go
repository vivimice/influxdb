...
func TestWindowFirstArrayCursor(t *testing.T) {
	testcases := []aggArrayCursorTest{
			...
			want: []*cursors.IntegerArray{
				{
					Timestamps: []int64{
						mustParseTime("2010-01-01T00:00:00Z").UnixNano(),
						mustParseTime("2010-01-01T00:14:00Z").UnixNano(),
						mustParseTime("2010-01-01T00:29:00Z").UnixNano(),
						mustParseTime("2010-01-01T00:44:00Z").UnixNano(),
						mustParseTime("2010-01-01T00:59:00Z").UnixNano(),
					},
		...
		{
			name:   "empty negative offset windows",
			every:  time.Minute,
			offset: -1 * time.Minute,
			inputArrays: []*cursors.IntegerArray{
				makeIntegerArray(
